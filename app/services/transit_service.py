...
_FACILITIES_SEED_PATH = Path(__file__).resolve().parent.parent / "data" / "facilities_seed.json"


def _load_facilities_seed() -> dict[str, list[tuple[str, str | None, str]]]:
    if not _FACILITIES_SEED_PATH.exists():
        logger.warning("facilities_seed.json not found at %s", _FACILITIES_SEED_PATH)
        return {}
    raw = json.loads(_FACILITIES_SEED_PATH.read_text(encoding="utf-8"))
    default_items = raw.pop("_default", [])
    seed: dict[str, list[tuple[str, str | None, str]]] = {}
    for station_name, items in raw.items():
        seed[station_name] = [(t, loc, status) for t, loc, status in items]
    seed["_default"] = [(t, loc, status) for t, loc, status in default_items]
    return seed


_STATIC_FACILITIES_SEED = _load_facilities_seed()
_DEFAULT_FACILITIES = _STATIC_FACILITIES_SEED.pop("_default", [])


def _build_static_facilities(normalized_station: str) -> StationFacilities:
    items = _STATIC_FACILITIES_SEED.get(normalized_station, _DEFAULT_FACILITIES)
    facilities = [
        StationFacility(
